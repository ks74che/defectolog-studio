...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      description: 'Короткий текст (до 200 символов). Показывается в карточке статьи в списке.',
      type: 'text',
      rows: 3,
    ...
    defineField({
      ...
      title: 'Категория',
      description:
        'Выберите категорию из списка. Категории создаются в разделе «Категория статей».',
      type: 'reference',
      to: [{type: 'category'}],
      validation: (Rule) => Rule.required(),
    }),
          ...
          fields: [
            {
              ...
              description: 'Коротко опишите, что на картинке. Нужно для поисковиков.',
            },
          ],
        }),
        ...
        defineArrayMember({
          ...
          preview: {
            ...
            prepare({title, category, date, media}) {
              ...
              const cat = category ? `📁 ${category}` : '⚠️ Без категории'
              return {
                title: title ?? 'Без названия',
                subtitle: dateStr ? `${cat} · ${dateStr}` : cat,
                media,
              }
            },
          },
        ...
        defineArrayMember({
          ...
          preview: {
            ...
            prepare({image, text}: {image?: unknown; text?: any[]}) {
              ...
              const preview = firstBlock?.children?.[0]?.text ?? 'Текст с картинкой'
              return {
                title: '🖼️ Текст с картинкой',
                subtitle: preview,
                media: image as any,
